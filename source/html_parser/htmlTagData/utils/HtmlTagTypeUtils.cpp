...
Type Html::Tag::GetTagType(const std::string_view &tag)
{
    ...
    if (Utils::FindIC(tag, TypeData<Type::Comment>::TagStart, tag.length()) != std::string_view::npos)
    {
        return Type::Comment;
    }

    if (Utils::FindIC(tag, TypeData<Type::Script>::TagStart, tag.length()) != std::string_view::npos)
    {
        return Type::Script;
    }
    ...
    if (Utils::FindIC(tag, TypeData<Type::NoScript>::TagStart, tag.length()) != std::string_view::npos)
    {
